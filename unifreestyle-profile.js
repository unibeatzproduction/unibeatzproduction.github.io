...
(function(){
  ...
  // FIX: Scoped video box getters — no more getElementById collision between host box and viewer modal
  function getHostVideoBox(){ return document.querySelector('#ubProfileLiveCamBox #ubHostVideoBox'); }
  function getViewerVideoBox(){ return document.querySelector('#ubProfileLiveModal #ubViewerVideoBox'); }
  ...
  async function connectProfileMedia(target, role){
    ...
    room.on('trackSubscribed',function(track,pub,participant){
      if(track.kind==='video'){
        // FIX: viewer uses scoped box getter with retry loop
        var tryAttach = function(attempts){
          ...
          if(box){
            ...
            console.warn('[profile] viewer video box never appeared after retries');
          }
        };
        tryAttach(15); // 15 x 300ms = 4.5 seconds
      }
