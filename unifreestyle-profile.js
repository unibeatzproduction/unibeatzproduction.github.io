...
(function(){
  ...
  function ensureHostChatBox(name){
    ...
    box.querySelector('#ubHostChatSend').onclick=function(){
      ...
      sendLiveChat(name, msg); input.value='';
    };
    box.querySelector('#ubHostChatInput').addEventListener('keydown',function(e){
      if(e.key==='Enter'){ var msg=e.target.value.trim(); if(!msg) return; sendLiveChat(name,msg); e.target.value=''; }
    });
  ...
  async function sendLiveChat(target, msg){
    var fb=getFb(); if(!fb) return;
    // Use liveSt.viewerTarget as fallback if target not passed
  ...
  async function listenLiveChat(target){
    var fb=getFb(); if(!fb) return;
    if(liveSt.chatUnsub) try{ liveSt.chatUnsub(); }catch(e){}
    // Scope to modal to avoid getElementById finding wrong element
  ...
  function ensureDMModal(){
    ...
    m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99999;display:none;flex-direction:column;color:#fff;';
    m.innerHTML=[
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid rgba(64,208,255,.25);background:rgba(3,3,5,.96);">',
        '<div style="font-family:Bebas Neue,Arial,sans-serif;font-size:1.6rem;letter-spacing:2px;color:#F0C040;">MESSAGES</div>',
        '<button id="ubDMClose" style="border:0;background:transparent;color:#F0C040;font-size:1.4rem;cursor:pointer;">×</button>',
      '</div>',
      '<div style="display:flex;flex:1;overflow:hidden;">',
        '<div id="ubDMThreadList" style="width:140px;flex-shrink:0;border-right:1px solid rgba(64,208,255,.18);overflow-y:auto;padding:8px 0;"></div>',
        '<div style="flex:1;display:flex;flex-direction:column;overflow:hidden;">',
          '<div id="ubDMFeed" style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:8px;">',
            '<div style="color:rgba(240,237,232,.45);font-size:.85rem;text-align:center;margin:auto;">Select a conversation or start a new one.</div>',
          '</div>',
          '<div style="padding:10px;border-top:1px solid rgba(64,208,255,.18);display:grid;grid-template-columns:1fr auto;gap:8px;">',
            '<input id="ubDMInput" maxlength="500" placeholder="Send a message..." style="background:#05070d;border:1px solid rgba(64,208,255,.45);border-radius:10px;color:#fff;padding:10px;outline:none;">',
            '<button id="ubDMSend" style="border:0;border-radius:10px;background:linear-gradient(135deg,#8B6914,#C9A84C,#F0C040);color:#030305;font-family:Orbitron,sans-serif;font-size:.52rem;font-weight:900;letter-spacing:1.5px;padding:0 14px;cursor:pointer;">SEND</button>',
          '</div>',
        '</div>',
      '</div>'
    ].join('');
    document.body.appendChild(m);
    m.querySelector('#ubDMClose').onclick=function(){ m.style.display='none'; if(_dmUnsub){ try{_dmUnsub();}catch(e){} _dmUnsub=null; } };
  ...
  function loadDMThreadList(){
    ...
    // Find all DM rooms involving me
    fb.getDocs(fb.collection(fb.db,'dm_rooms')).then(function(snap){
      ...
      snap.forEach(function(doc){
        ...
        if(id.indexOf(me)>-1){
          ...
          if(other) threads.push({ roomId:id, other:other });
        }
      });
      list.innerHTML='';
      // New DM button
      var newBtn=document.createElement('div');
      newBtn.style.cssText='padding:8px 10px;cursor:pointer;border-bottom:1px solid rgba(64,208,255,.1);font-family:Orbitron,sans-serif;font-size:.42rem;color:#40D0FF;letter-spacing:1px;';
      newBtn.textContent='+ NEW DM';
      newBtn.onclick=function(){
        var t=prompt('Enter username to DM:'); if(!t) return;
        openDMThread(clean(t));
      };
      list.appendChild(newBtn);
      threads.forEach(function(th){
        var row=document.createElement('div');
        row.style.cssText='padding:10px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,.05);font-size:.82rem;';
        row.textContent='@'+th.other;
        row.onclick=function(){ openDMThread(th.other); };
        list.appendChild(row);
      });
      if(!threads.length){
        var empty=document.createElement('div');
        empty.style.cssText='padding:10px;color:rgba(240,237,232,.4);font-size:.75rem;';
  ...
  function openDMThread(target){
    ...
    document.querySelectorAll('#ubDMThreadList div').forEach(function(el){
      el.style.background=el.textContent==='@'+target?'rgba(64,208,255,.1)':'';
    });
    // Ensure room doc exists
    fb.setDoc(fb.doc(fb.db,'dm_rooms',roomId),{ participants:[me,target], updatedAt:Date.now() },{merge:true}).catch(function(){});
    // Listen to messages
    ...
    _dmUnsub=fb.onSnapshot(q,function(snap){
      ...
      snap.forEach(function(doc){
        ...
        var isMe=d.from===me;
        var row=document.createElement('div');
        row.style.cssText='max-width:80%;padding:8px 12px;border-radius:12px;font-size:.88rem;line-height:1.35;'+(isMe?'align-self:flex-end;background:rgba(64,208,255,.18);border:1px solid rgba(64,208,255,.3);':'align-self:flex-start;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);');
        row.innerHTML=(isMe?'':'<div style="font-family:Orbitron,sans-serif;font-size:.4rem;color:#40D0FF;margin-bottom:3px;">@'+esc(d.from)+'</div>')+esc(d.text);
        feed.appendChild(row);
      });
      feed.scrollTop=feed.scrollHeight;
    });
  ...
  function sendDM(){
    ...
    fb.addDoc(fb.collection(fb.db,'dm_rooms',roomId,'messages'),
      ...
      // Update room's updatedAt for thread list ordering
      fb.setDoc(fb.doc(fb.db,'dm_rooms',roomId),{ updatedAt:Date.now() },{merge:true}).catch(function(){});
