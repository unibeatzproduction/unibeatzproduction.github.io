...
(function(){
  ...
  // ═══════════════════════════════════════════════════
  var liveSt = { room: null, role: null, chatUnsub: null, viewerTarget: null };
  ...
  function getLiveVideoBox(){ return document.getElementById('ubLiveVideoBox'); }
  ...
  function attachProfileVideo(track, identity, local){
    var box = getLiveVideoBox(); if(!box||!track) return;
    box.innerHTML='';
    var vid=document.createElement('video');
    vid.dataset.profileLivekit='yes';
    styleProfileVideo(vid,local);
    box.appendChild(vid);
    track.attach(vid);
  ...
  async function connectProfileMedia(target, role){
    ...
    room.on('trackSubscribed',function(track,pub,participant){
      if(track.kind==='video') attachProfileVideo(track,participant.identity,false);
    ...
    if(role==='host'){
      ...
      room.localParticipant.trackPublications.forEach(function(pub){
        if(pub.track&&pub.track.kind==='video') attachProfileVideo(pub.track,identity,true);
      });
      toast('📹 Profile live cam + mic connected');
    } else {
      var box=getLiveVideoBox();
      if(box) box.innerHTML='<div style="padding:18px;text-align:center;color:#40D0FF;font-family:Orbitron,sans-serif;font-size:.55rem;">Connecting to live camera...</div>';
  ...
  async function setLive(isLive){
    ...
    if(isLive) await connectProfileMedia(name,'host').catch(function(e){ toast('⚠️ Cam failed: '+e.message); });
    else disconnectProfileMedia();
  ...
  async function follow(target){
    ...
    var fb=getFb();
    if(fb){
      await fb.setDoc(fb.doc(fb.db,'profile_follows',me+'_'+target),
        { follower:me, following:target, at:Date.now() },{merge:true});
    }
  ...
  function ensureLiveModal(){
    ...
    m.innerHTML='<div style="max-width:1000px;margin:0 auto;"><button id="ubLiveClose" class="btn btn-gold" style="width:auto;margin-bottom:12px;">← CLOSE</button><div id="ubLiveHeader"></div><div id="ubLiveVideoBox" style="width:100%;min-height:280px;border-radius:14px;border:1px solid rgba(64,208,255,.45);background:#05070d;display:flex;align-items:center;justify-content:center;color:#40D0FF;font-family:Orbitron,sans-serif;font-size:.55rem;margin-bottom:12px;position:relative;overflow:hidden;aspect-ratio:9/16;max-height:55vh;"></div><div style="border-radius:14px;border:1px solid rgba(201,168,76,.45);background:rgba(0,0,0,.35);padding:12px;"><div style="font-family:Orbitron,sans-serif;color:#40D0FF;font-size:.55rem;letter-spacing:2px;margin-bottom:8px;">LIVE CHAT</div><div id="ubLiveChatList" style="height:160px;overflow:auto;border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:8px;margin-bottom:8px;"></div><input id="ubLiveChatInput" placeholder="Talk live..." style="width:100%;padding:10px;border-radius:8px;border:1px solid rgba(64,208,255,.5);background:#05070d;color:#fff;margin-bottom:8px;box-sizing:border-box;"><button id="ubLiveSend" class="btn btn-blue">SEND CHAT</button></div></div>';
  ...
  // Keep backward compat with scripts that call ubProfileLive or ubProfileFollow
