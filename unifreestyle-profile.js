...
(function(){
  ...
  async function connectProfileMedia(target, role){
    ...
    room.on('trackSubscribed',function(track,pub,participant){
      if(track.kind==='video'){
        // Wait for box to exist then attach
      ...
      room.localParticipant.trackPublications.forEach(function(pub){
        if(pub.track&&pub.track.kind==='video'){
          ...
          if(box){
            ...
            vid.setAttribute('playsinline','true');
            vid.style.cssText='position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:#000;transform:scaleX(-1);';
            vid.dataset.profileLivekit='yes';
            box.style.position='relative';
            box.appendChild(vid);
  ...
  async function setLive(isLive){
    ...
    if(isLive){
      // Inject live cam preview box into profile page
      ensureProfileCamBox();
