...
(function(){
  ...
  function attachViewerVideo(track){
    ...
    console.log('[profile] attachViewerVideo called, box found:', !!box);
    ...
    box.appendChild(vid);
    track.attach(vid);
    vid.play().catch(function(e){ console.warn('[profile] viewer video play failed:', e); });
    console.log('[profile] viewer video element attached and play() called');
  }
  ...
  async function connectProfileMedia(target, role){
    ...
    if(role==='host'){
      ...
      // FIX: also scan already-published tracks from existing participants
      // trackSubscribed may have fired during connect() before our listener registered on some mobile browsers,
      // OR the host published before viewer joined — either way we need to check existing publications.
      setTimeout(function(){
        room.remoteParticipants.forEach(function(participant){
          participant.trackPublications.forEach(function(pub){
            if(pub.isSubscribed && pub.track){
              if(pub.track.kind==='video'){
                console.log('[profile] found existing video track on participant:', participant.identity);
                attachViewerVideo(pub.track);
              }
              if(pub.track.kind==='audio'){
                attachProfileAudio(pub.track, participant.identity);
              }
            }
          });
        });
      }, 800); // small delay so DOM is stable
    }
    return room;
  }
