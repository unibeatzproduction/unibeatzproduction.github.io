...
// Works with profile lives and battle rooms without changing core app flow.
(function(){
  'use strict';

  var st = { fb:null, db:null, unsub:null, activeRoom:null, slow:false, lastSend:0 };
  ...
  function inferRoom(){
    ...
    if(id==='page-battle-live') return 'battle_live';
    if(id==='page-profile') return 'profile_'+uname();
    if(id==='page-home') return 'home_live';
    if(id==='page-practice') return 'practice_'+uname();
    return id.replace('page-','') || 'general';
  }
  ...
  function ensurePanel(){
    ...
    if(!launch){
      ...
      launch.className='ub-live-chat-launch';
      launch.innerHTML='💬';
    ...
    panel.innerHTML='<div class="ub-live-chat-head"><div><div class="ub-live-chat-title">LIVE CHAT</div><div class="ub-live-chat-room" id="ubLiveChatRoom">GENERAL</div></div><button class="ub-live-chat-close" id="ubLiveChatClose">×</button></div><div class="ub-live-chat-feed" id="ubLiveChatFeed"></div><div class="ub-live-chat-emoji"><button data-emoji="🔥">🔥</button><button data-emoji="⚡">⚡</button><button data-emoji="👑">👑</button><button data-emoji="🎤">🎤</button><button data-emoji="💎">💎</button><button data-emoji="😂">😂</button><button data-emoji="👏">👏</button></div><div class="ub-live-chat-input"><input id="ubLiveChatInput" maxlength="220" placeholder="Say something live..."><button id="ubLiveChatSend">SEND</button></div>';
  ...
  async function open(room){
    await fb();
    room=String(room||inferRoom()).replace(/[^a-zA-Z0-9_-]/g,'_');
    st.activeRoom=room;
    var title=document.getElementById('ubLiveChatRoom');
    if(title) title.textContent=room.replace(/_/g,' ').toUpperCase();
    if(st.unsub) try{ st.unsub(); }catch(e){}
    var q=st.fb.query(st.fb.collection(st.db,'live_chats',room,'messages'),st.fb.orderBy('at','asc'),st.fb.limit(80));
  ...
  async function send(text,type){
    ...
    if(!text) return;
    var room=st.activeRoom || inferRoom();
    st.lastSend=now;
  ...
  window.ubLiveChat={ open:open, toggle:toggleChat, send:send, reaction:sendReaction, slow:function(on){st.slow=!!on;} };
