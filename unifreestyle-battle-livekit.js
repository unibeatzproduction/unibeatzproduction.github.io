...
// Keeps Cypher separate. Designed for Instant, Live Battle, Ranked, and Practice.
...
(function(){
  ...
  function resolveUsername(role){
    var u = currentUser();
    return (u && (u.username || u.name)) || (role || 'user') + '_' + Math.floor(Math.random() * 9999);
  }
  ...
  function findContainer(role, identity){
    ...
    if(r === 'artist1' || r === 'artist-1') selectors.push('#artist1Video','#artist1Cam','#artistOneVideo','#battleArtist1Video','.artist-1-video','.artist1-video','[data-battle-video="artist1"]');
    if(r === 'artist2' || r === 'artist-2') selectors.push('#artist2Video','#artist2Cam','#artistTwoVideo','#battleArtist2Video','.artist-2-video','.artist2-video','[data-battle-video="artist2"]');
    ...
    selectors.push('#liveBattleStage','#battleLiveStage','#page-live .page-body','#page-battle .page-body','#page-instant .page-body','#page-practice .page-body');
  ...
  function makeFallbackStage(role, identity){
    var page = document.querySelector('#page-live .page-body, #page-battle .page-body, #page-instant .page-body, #page-practice .page-body, main, body');
  ...
  function attachVideoTo(container, track, identity, local){
    ...
    vid.id = id;
    vid.dataset.ubBattleLivekit = 'yes';
  ...
  async function connectBattleLive(role, roomName){
    ...
    role = String(role || 'artist1').toLowerCase();
    roomName = roomName || 'battle-room';
    st.role = role;
    st.battleRoom = roomName;
    st.username = resolveUsername(role);

    var LK = await waitForLiveKit();
    var url = TOKEN_FN + '?room=' + encodeURIComponent(roomName) + '&username=' + encodeURIComponent(st.username + '-' + role);
    ...
    room.on('trackSubscribed', function(track, publication, participant){
      ...
      if(track.kind === 'video'){
        var targetRole = participant.identity.indexOf('artist2') !== -1 ? 'artist2' : participant.identity.indexOf('dj') !== -1 ? 'dj' : 'artist1';
        attachVideoTo(findContainer(targetRole, participant.identity), track, participant.identity, false);
      }
    });

    room.on('trackUnsubscribed', function(track, publication, participant){
      var v = document.getElementById('ub-livekit-video-' + safeId(participant.identity));
      if(v) v.remove();
      var a = document.getElementById('ub-livekit-audio-' + safeId(participant.identity));
      if(a) a.remove();
    });
    ...
    if(role !== 'viewer' && role !== 'watch'){
      await room.localParticipant.enableCameraAndMicrophone();
      room.localParticipant.trackPublications.forEach(function(pub){
        if(pub.track && pub.track.kind === 'video') attachVideoTo(findContainer(role, st.username), pub.track, st.username + '-' + role, true);
      });
    }

    toast('📹 Battle Live connected: ' + role);
    return room;
  }
  ...
  async function toggleBattleMic(){
    ...
    st.micOn = !st.micOn;
    await st.room.localParticipant.setMicrophoneEnabled(st.micOn);
  ...
  async function toggleBattleCam(){
    if(!st.room) return toast('Not connected');
    st.camOn = !st.camOn;
    await st.room.localParticipant.setCameraEnabled(st.camOn);
    if(st.camOn){
      st.room.localParticipant.trackPublications.forEach(function(pub){
        if(pub.track && pub.track.kind === 'video') attachVideoTo(findContainer(st.role, st.username), pub.track, st.username + '-' + st.role, true);
      });
    }
