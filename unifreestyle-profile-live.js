...
// Live profile foundation: go live, homepage live rail, follow, and live chat.
(function(){
  'use strict';
  ...
  async function setLive(isLive){
    ...
    await st.fb.setDoc(st.fb.doc(st.db,'live_profiles',name),{
      username:name, displayName:u.name||name, role:u.role||'artist', avatar:u.avatar||'🎤', photo:u.photo||'',
  ...
  function homeRail(){
    ...
    box.style.cssText='margin:16px 0;padding:14px;border-radius:16px;border:1px solid rgba(64,208,255,.35);background:rgba(0,0,0,.28);color:#fff;';
    box.innerHTML='<div style="font-family:Orbitron,sans-serif;font-size:.5rem;letter-spacing:2px;color:#40D0FF;margin-bottom:6px;">LIVE PROFILES</div><div style="font-family:Bebas Neue,Arial,sans-serif;font-size:1.7rem;letter-spacing:2px;color:#F0C040;">WATCH LIVE NOW</div><div id="ubLiveProfilesList" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-top:12px;"></div>';
  ...
  async function refreshLiveProfiles(){
    ...
    list.innerHTML='';
    if(snap.empty){ list.innerHTML='<div style="color:rgba(240,237,232,.65);font-size:.86rem;">No one is live yet.</div>'; return; }
    snap.forEach(function(doc){
      var p=doc.data();
      var card=document.createElement('div');
      card.style.cssText='padding:12px;border-radius:12px;border:1px solid rgba(201,168,76,.35);background:rgba(0,0,0,.35);cursor:pointer;';
      card.innerHTML='<div style="display:flex;align-items:center;gap:10px;"><div style="width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:1px solid #F0C040;font-size:1.4rem;overflow:hidden;">'+(p.photo?'<img src="'+esc(p.photo)+'" style="width:100%;height:100%;object-fit:cover;">':esc(p.avatar||'🎤'))+'</div><div><div style="color:#F0C040;font-family:Bebas Neue;font-size:1.2rem;letter-spacing:1.5px;">'+esc(p.displayName||p.username)+'</div><div style="color:#40D0FF;font-family:Orbitron;font-size:.45rem;letter-spacing:1.5px;">🔴 LIVE · @'+esc(p.username)+'</div></div></div>';
