...
/* ===== PRO 4 ACTIONS ===== */
(function(){
 const OS=window.UniBeatzDJOS;if(!OS)return;OS.version='4.0.0';
 const A=()=>window.UniBeatzRadioDeck,p=s=>s.noteOn!==false,d=s=>Number.isFinite(s.relativeDelta)?s.relativeDelta:(s.value<64?s.value:s.value-128);
 ['A','B'].forEach(x=>{
  [1,2,3,4].forEach(i=>OS.registerAction(`deck${x}.hotCue${i}`,s=>{if(p(s))A()?.hotCue(x,i-1)},`Deck ${x} Hot Cue ${i}`));
  [1,2,4,8].forEach(i=>OS.registerAction(`deck${x}.loop${i}`,s=>{if(p(s))A()?.loop(x,i)},`Deck ${x} Loop ${i}`));
  OS.registerAction(`deck${x}.loopOff`,s=>{if(p(s))A()?.loopOff(x)},`Deck ${x} Loop Off`);
  OS.registerAction(`deck${x}.shift`,s=>A()?.shift(x,!!s.noteOn&&!s.noteOff),`Deck ${x} Shift`);
  OS.registerAction(`deck${x}.headphoneCue`,s=>{if(p(s))A()?.headphoneCue(x)},`Deck ${x} Headphone Cue`);
  ['Echo','Reverb','Filter','Stutter'].forEach(n=>OS.registerAction(`deck${x}.fx${n}`,s=>A()?.fx(x,n.toLowerCase(),s.normalized),`Deck ${x} FX ${n}`));
  [['Full','full'],['Vocals','vocals'],['Instrumental','instrumental'],['Drums','drums'],['Bass','bass'],['Other','other']].forEach(([n,k])=>OS.registerAction(`deck${x}.stem${n}`,s=>{if(p(s))A()?.stems[k](x)},`Deck ${x} Stem ${n}`));
  OS.registerAction(`deck${x}.jog`,s=>A()?.jog(x,d(s)),`Deck ${x} Jog Wheel`);
  OS.registerAction(`deck${x}.play`,s=>{if(p(s))A()?.play(x)},`Deck ${x} Play`);
  OS.registerAction(`deck${x}.cue`,s=>{if(p(s))A()?.cue(x)},`Deck ${x} Cue`);
 });
 OS.registerAction('browser.encoder',s=>A()?.browserMove(d(s)>0?1:-1),'Track Browser Encoder');
 OS.registerAction('browser.previous',s=>{if(p(s))A()?.browserMove(-1)},'Track Browser Previous');
 OS.registerAction('browser.next',s=>{if(p(s))A()?.browserMove(1)},'Track Browser Next');
 OS.registerAction('browser.loadA',s=>{if(p(s))A()?.browserLoad('A')},'Load Selected to Deck A');
 OS.registerAction('browser.loadB',s=>{if(p(s))A()?.browserLoad('B')},'Load Selected to Deck B');
 OS.registerAction('headphones.volume',s=>A()?.headphoneVolume(s.normalized),'Headphone Volume');
 if(OS.ui?.render){document.getElementById('ubDjOSPanel')?.remove();OS.ui.build();OS.ui.render()}
 console.log('[UniBeatz DJ OS] V4.0.0 Pro ready');
})();
