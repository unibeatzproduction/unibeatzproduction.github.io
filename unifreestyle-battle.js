...
(function(){
  ...
  function ok(){
    return !!document.querySelector('meta[name="ub-platform"][content="battle"]') ||
      location.pathname.toLowerCase().includes('unifreestyle.html');
  }
  ...
  // ═══════════════════════════════════════════════════
  // LOCKED JUDGMENT SYSTEM — DJ 45 / VIEWERS 40 / AI 15
  // Money support is intentionally NOT counted as a vote.
  // ═══════════════════════════════════════════════════
  var JUDGMENT_WEIGHTS = Object.freeze({ dj:0.45, viewers:0.40, ai:0.15 });
  ...
  function calculateJudgment(input){
    input=input||{};
    var dj=clampScore(input.dj);
    var viewers=clampScore(input.viewers);
    var ai=clampScore(input.ai);
    return {
      dj:dj,
      viewers:viewers,
      ai:ai,
      weights:{dj:45,viewers:40,ai:15},
      final:Number((dj*JUDGMENT_WEIGHTS.dj + viewers*JUDGMENT_WEIGHTS.viewers + ai*JUDGMENT_WEIGHTS.ai).toFixed(2))
    };
  }

  async function saveJudgment(roomId,artistId,input){
    var fb=getFb();
    if(!fb) throw new Error('Firebase is not ready');
    roomId=safeId(roomId||liveSt.battleRoom||modeState.room||'battle-room');
    artistId=safeId(artistId||'artist');
    var result=calculateJudgment(input);
    var judge=currentUser()||{};
    await fb.setDoc(
      fb.doc(fb.db,'battle_judgments',roomId+'_'+artistId),
      {
        roomId:roomId,
        artistId:artistId,
        djScore:result.dj,
        viewerScore:result.viewers,
        aiScore:result.ai,
        finalScore:result.final,
        weights:result.weights,
        submittedBy:judge.username||judge.name||judge.uid||'system',
...
function watchQueueForBanner(sessionId) {
  ...
  window.ubBattle = {
    judgmentWeights:{dj:45,viewers:40,ai:15},
    calculateJudgment:calculateJudgment,
