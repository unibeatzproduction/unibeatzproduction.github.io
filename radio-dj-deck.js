...
function loadTo(deck, item){
  ...
  const label=deck==='A'?deckALabel:deckBLabel;
  audio.pause();
  ...
  audio.load();
  label.textContent=deck+': '+itemName(item).slice(0,20);
  window.__ubLoadedDeckItems=window.__ubLoadedDeckItems||{A:null,B:null};
  window.__ubLoadedDeckItems[deck]=item;
  window.dispatchEvent(new CustomEvent('ub-radio-track-loaded',{detail:{deck,item,url}}));
...
async function loadQueue(){
  ...
  try{
    ...
    window.UniBeatzRadioQueue=queue;
...
// ===== UNIBEATZ RADIO DECK PRO 4 =====
const UBPRO={A:{item:null,peaks:null,cues:[null,null,null,null],loop:null,shift:false,stemPlayers:{}},
B:{item:null,peaks:null,cues:[null,null,null,null],loop:null,shift:false,stemPlayers:{}},
browser:0,headphone:{A:false,B:false,volume:.75}};

function ubAudio(d){return d==='A'?deckA:deckB}
function ubCanvas(d){return document.getElementById(d==='A'?'waveCanvasA':'waveCanvasB')}
function ubFmt(t){t=Math.max(0,Number(t)||0);return Math.floor(t/60)+':'+String(Math.floor(t%60)).padStart(2,'0')}
async function ubDecode(d,url){
 try{
  const ctx=getEqCtx(); if(ctx.state==='suspended') await ctx.resume();
  ...
  const data=buf.getChannelData(0), count=900, block=Math.max(1,Math.floor(data.length/count));
  const peaks=new Float32Array(count);
  for(let i=0;i<count;i++){let p=0;for(let j=i*block;j<Math.min(data.length,(i+1)*block);j++)p=Math.max(p,Math.abs(data[j]));peaks[i]=p}
  UBPRO[d].peaks=peaks;
 }catch(e){console.warn('[waveform]',e);UBPRO[d].peaks=null}
}
function ubDraw(d){
 const c=ubCanvas(d),a=ubAudio(d); if(!c)return;
 const r=window.devicePixelRatio||1,w=Math.max(1,c.clientWidth),h=Math.max(1,c.clientHeight);
 c.width=w*r;c.height=h*r;const x=c.getContext('2d');x.setTransform(r,0,0,r,0,0);
 x.fillStyle='#07070c';x.fillRect(0,0,w,h);
 const p=UBPRO[d].peaks,col=d==='A'?'#40D0FF':'#F0C040';
 if(p){x.fillStyle=col;const step=p.length/w;for(let i=0;i<w;i++){const q=p[Math.min(p.length-1,Math.floor(i*step))],amp=Math.max(1,q*h*.45);x.fillRect(i,h/2-amp,1,amp*2)}}
 if(a.duration&&isFinite(a.duration)){const k=Math.max(0,Math.min(1,a.currentTime/a.duration));x.fillStyle='rgba(255,255,255,.12)';x.fillRect(0,0,w*k,h);x.fillStyle='#fff';x.fillRect(w*k,0,2,h)}
 UBPRO[d].cues.forEach((t,i)=>{if(t==null||!a.duration)return;x.fillStyle=i%2?'#F0C040':'#40D0FF';x.fillRect((t/a.duration)*w,0,2,h)})
}
function ubTick(){
 ['A','B'].forEach(d=>{const a=ubAudio(d),l=UBPRO[d].loop;if(l&&a.currentTime>=l.end)a.currentTime=l.start;const t=document.getElementById('time'+d);if(t)t.textContent=ubFmt(a.currentTime);ubDraw(d)});
 requestAnimationFrame(ubTick)
}
window.addEventListener('ub-radio-track-loaded',e=>{const {deck,item,url}=e.detail;UBPRO[deck].item=item;UBPRO[deck].cues=[null,null,null,null];UBPRO[deck].loop=null;ubDecode(deck,url)});
function ubBrowser(delta){const list=window.UniBeatzRadioQueue||[];if(!list.length)return note('No approved tracks in browser.','#F0C040');UBPRO.browser=(UBPRO.browser+delta+list.length)%list.length;qList?.querySelectorAll('.tb-row').forEach((r,i)=>{r.style.outline=i===UBPRO.browser?'2px solid #40D0FF':'none';if(i===UBPRO.browser)r.scrollIntoView({block:'nearest'})});note('Selected: '+itemName(list[UBPRO.browser]))}
function ubLoad(d){const item=(window.UniBeatzRadioQueue||[])[UBPRO.browser];if(item)loadTo(d,item)}
function ubCue(d,i){const a=ubAudio(d),s=UBPRO[d];if(!a.src)return;if(s.shift){s.cues[i]=null}else if(s.cues[i]==null){s.cues[i]=a.currentTime}else{a.currentTime=s.cues[i];a.play().catch(()=>{})}ubDraw(d)}
function ubLoop(d,b){const a=ubAudio(d),bpm=Number(UBPRO[d].item?.bpm||120),len=(60/bpm)*b;UBPRO[d].loop={start:a.currentTime,end:a.currentTime+len};note('Deck '+d+' loop '+b+' beats')}
function ubJog(d,v){const a=ubAudio(d),scale=UBPRO[d].shift?.45:.035;a.currentTime=Math.max(0,Math.min(a.duration||Infinity,a.currentTime+v*scale))}
function ubShift(d,on){UBPRO[d].shift=!!on}
function ubHead(d){UBPRO.headphone[d]=!UBPRO.headphone[d];note('Headphone Cue '+d+' '+(UBPRO.headphone[d]?'ON':'OFF'))}
function ubHeadVol(v){UBPRO.headphone.volume=Math.max(0,Math.min(1,v));note('Headphone volume '+Math.round(v*100)+'%')}
function ubStopStems(d){Object.values(UBPRO[d].stemPlayers).forEach(a=>{try{a.pause();a.remove()}catch(e){}});UBPRO[d].stemPlayers={}}
function ubStem(d,mode){
 const item=UBPRO[d].item||{},st=item.stems||{},urls={vocals:st.vocalsUrl||item.vocalsUrl||'',instrumental:st.instrumentalUrl||item.instrumentalUrl||'',drums:st.drumsUrl||item.drumsUrl||'',bass:st.bassUrl||item.bassUrl||'',other:st.otherUrl||item.otherUrl||''};
 const full=ubAudio(d);if(mode==='full'){ubStopStems(d);full.muted=false;full.play().catch(()=>{});return}
 let names=mode==='instrumental'?(urls.instrumental?['instrumental']:['drums','bass','other']):[mode];names=names.filter(n=>urls[n]);
 if(!names.length)return note('No '+mode+' stem exists for this track.','#F0C040');
 const time=full.currentTime,playing=!full.paused;full.pause();full.muted=true;ubStopStems(d);
 names.forEach(n=>{const a=new Audio(urls[n]);a.crossOrigin='anonymous';a.currentTime=time;a.volume=full.volume;a.playbackRate=full.playbackRate;document.body.appendChild(a);UBPRO[d].stemPlayers[n]=a;if(playing)a.play().catch(()=>{})});
 note('Deck '+d+' '+mode.toUpperCase()+' stem active.','#5dff9e')
}
window.UniBeatzRadioDeck={state:UBPRO,browserMove:ubBrowser,browserLoad:ubLoad,hotCue:ubCue,loop:ubLoop,loopOff:d=>UBPRO[d].loop=null,jog:ubJog,shift:ubShift,headphoneCue:ubHead,headphoneVolume:ubHeadVol,play:d=>ubAudio(d).play().catch(e=>note('Play failed: '+e.message,'#ff7474')),cue:d=>{const a=ubAudio(d);a.currentTime=0;a.play().catch(()=>{})},fx:(d,n,v)=>window.dispatchEvent(new CustomEvent('ub-radio-fx',{detail:{deck:d,name:n,value:v}})),stems:{full:d=>ubStem(d,'full'),vocals:d=>ubStem(d,'vocals'),instrumental:d=>ubStem(d,'instrumental'),drums:d=>ubStem(d,'drums'),bass:d=>ubStem(d,'bass'),other:d=>ubStem(d,'other')}};
requestAnimationFrame(ubTick);
console.log('[UniBeatz Radio Deck Pro 4] ready');
