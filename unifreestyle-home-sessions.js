...
// Home cleanup: remove Instant Mode safely + live counts + standalone scheduled session rooms + Browser Producer room.
(function(){
  'use strict';
  ...
  function css(){
    ...
    s.textContent=[
      ...
      '.ub-session-room-card,.ub-producer-card{margin:14px;border:1px solid rgba(64,208,255,.32);border-radius:16px;background:linear-gradient(180deg,rgba(8,12,20,.96),rgba(3,3,5,.94));box-shadow:0 18px 45px rgba(0,0,0,.42);overflow:hidden;}',
      '.ub-session-room-hero,.ub-producer-hero{padding:18px 16px;border-bottom:1px solid rgba(201,168,76,.22);background:radial-gradient(circle at 15% 0%,rgba(64,208,255,.18),transparent 35%),radial-gradient(circle at 85% 25%,rgba(240,192,64,.14),transparent 35%);}',
      ...
      '.ub-room-grid,.ub-producer-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:14px;}','.ub-room-stat,.ub-prod-tile{border:1px solid rgba(64,208,255,.24);border-radius:12px;background:rgba(255,255,255,.035);padding:12px 6px;text-align:center;}',
      '.ub-room-stat b,.ub-prod-tile b{display:block;font-family:Bebas Neue,Arial,sans-serif;font-size:1.5rem;color:#F0C040;line-height:1;letter-spacing:1px;}','.ub-room-stat span,.ub-prod-tile span{display:block;margin-top:5px;font-family:Orbitron,sans-serif;font-size:.42rem;letter-spacing:1.3px;color:rgba(240,237,232,.62);}',
      '.ub-room-panel,.ub-producer-panel{margin:0 14px 14px;border:1px solid rgba(201,168,76,.22);border-radius:14px;padding:14px;background:rgba(0,0,0,.22);}',
      '.ub-room-panel-title{font-family:Orbitron,sans-serif;font-size:.5rem;letter-spacing:2px;color:#40D0FF;margin-bottom:9px;}','.ub-room-turn{font-family:Bebas Neue,Arial,sans-serif;font-size:1.5rem;letter-spacing:2px;color:#fff;}',
      '.ub-room-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:0 14px 14px;}','.ub-room-actions button,.ub-producer-actions button{border-radius:12px;padding:13px 8px;font-family:Orbitron,sans-serif;font-size:.52rem;letter-spacing:1.5px;font-weight:900;border:1px solid transparent;}',
      '.ub-producer-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px;}','.ub-btn-gold{background:linear-gradient(135deg,#8B6914,#C9A84C,#F0C040);color:#030305;}','.ub-btn-blue{background:rgba(64,208,255,.12);color:#40D0FF;border-color:rgba(64,208,255,.34)!important;}',
      '@media(max-width:520px){.ub-room-title{font-size:1.75rem}.ub-room-grid,.ub-producer-grid{grid-template-columns:1fr}.ub-room-actions,.ub-producer-actions{grid-template-columns:1fr}}'
    ].join('');
    document.head.appendChild(s);
  }

  function ensureStandaloneRooms(){
    if(!document.getElementById('page-openfreestyle')){
  ...
  function ensureBrowserProducerPage(){
    if(document.getElementById('page-browserproducer'))return;
    var profile=document.getElementById('page-profile')||document.getElementById('page-home'); if(!profile)return;
    profile.insertAdjacentHTML('afterend','<div class="page" id="page-browserproducer"><div class="top-bar"><button class="icon-btn" onclick="goToPage(\'profile\')">←</button><h1>🎹 Browser Producer</h1><div class="icon-btn" onclick="goToPage(\'profile\')">👤</div></div><div class="page-body"><div class="ub-producer-card"><div class="ub-producer-hero"><span class="ub-room-live" style="border-color:#40D0FF;color:#40D0FF;background:rgba(64,208,255,.12);">BROWSER TOOL</span><div class="ub-room-title">BROWSER PRODUCER</div><div class="ub-room-sub">Create, test, and preview producer tools without entering Beat Admin.</div></div><div class="ub-producer-grid"><div class="ub-prod-tile"><b>🎛️</b><span>MIXER</span></div><div class="ub-prod-tile"><b>🎹</b><span>KEYS</span></div><div class="ub-prod-tile"><b>🥁</b><span>DRUMS</span></div></div><div class="ub-producer-panel"><div class="ub-room-panel-title">PRODUCER WORKSPACE</div><div style="color:rgba(240,237,232,.72);line-height:1.5;font-size:.9rem;">This is the creator-facing Browser Producer room. Beat Admin stays separate for admin uploads only.</div><div class="ub-producer-actions"><button class="ub-btn-gold" onclick="showToast(\'🎹 Browser Producer tools coming next\')">OPEN PRODUCER</button><button class="ub-btn-blue" onclick="goToPage(\'profile\')">BACK PROFILE</button></div></div></div></div>'+navHtml('')+'</div>');
  }

  function patchBrowserProducerButton(){
    [].slice.call(document.querySelectorAll('button')).forEach(function(btn){
      if(norm(btn.textContent).indexOf('browser producer')>-1){
        btn.onclick=function(e){if(e){e.preventDefault();e.stopPropagation();}ensureBrowserProducerPage();if(typeof window.goToPage==='function')window.goToPage('browserproducer');return false;};
      }
    });
  }
  ...
  function openRoom(type){var s=DEFAULTS[type]||DEFAULTS.open;try{localStorage.setItem(ACTIVE_KEY,JSON.stringify({type:type,title:s.title,room:s.roomTitle,at:Date.now()}));}catch(e){}bump(type,1);var page=type==='beatkill'?'beatkill':'openfreestyle';if(typeof window.goToPage==='function')window.goToPage(page);else location.hash='#'+page;}
  ...
  function render(){if(!ok())return;css();ensureStandaloneRooms();removeInstantMode();forceTopProfileIcon();patchBrowserProducerButton();var cards=findSessionCards(),counts=currentCounts();enhanceCard('open',cards.open,counts.open);enhanceCard('beatkill',cards.beatkill,counts.beatkill);updateCount('open',counts.open);updateCount('beatkill',counts.beatkill);}
  ...
  window.ubHomeSessions={openRoom:openRoom,joinRoom:joinRoom,refresh:render,openBrowserProducer:function(){ensureBrowserProducerPage();if(window.goToPage)goToPage('browserproducer');}};
