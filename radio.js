...
function filteredTracks(){return currentGenre==='All' ? playbackQueue : playbackQueue.filter(t=>t.genre===currentGenre || isAnnouncement(t));}
function currentTrack(){return filteredTracks()[currentTrackIndex] || playbackQueue[0] || null;}
...
function buildPlaybackQueue(){
  const songs = [...allApprovedTracks].sort((a,b)=>Number(a.autoOrder ?? a.sortOrder ?? 9999)-Number(b.autoOrder ?? b.sortOrder ?? 9999));
  const assets = [...allRadioAssets].filter(a=>rule(a,'enabled',true)).sort((a,b)=>Number(a.autoOrder ?? a.sortOrder ?? 9999)-Number(b.autoOrder ?? b.sortOrder ?? 9999));
...
function updateReactionButtons(track){
  ...
  const likeBtn=document.getElementById('likeTrack'), dislikeBtn=document.getElementById('dislikeTrack');
...
async function loadReactionsForTrack(track){
  ...
  try{
    ...
    snap.forEach(d=>{const data=d.data()||{}; if(data.reaction==='like') likes++; if(data.reaction==='dislike') dislikes++; if(data.listenerId===listenerId) mine=data.reaction||'';});
...
async function saveReaction(reaction){
  ...
  try{
    ...
    await setDoc(doc(db,'radio_reactions',reactionId),{trackId:track.id,trackTitle:titleOf(track),artistName:artistOf(track),listenerId,reaction,uid:auth.currentUser?.uid||null,isAnonymous:!!auth.currentUser?.isAnonymous,updatedAt:serverTimestamp(),createdAt:serverTimestamp()},{merge:true});
  ...
  }catch(error){console.error('REACTION SAVE ERROR',error);nowPlayingBadge.textContent='REACTION ERROR — CHECK FIREBASE RULES';}
}

accountBtn?.addEventListener('click',()=>{if(window.UniBeatzAuth?.getUser?.()) window.UniBeatzAuth.showAccount(); else if(window.UniBeatzAuth?.showLogin) window.UniBeatzAuth.showLogin();});
...
form?.addEventListener('submit',async(e)=>{
  e.preventDefault();
  notice.textContent='Uploading track for moderation...'; notice.style.color='#40D0FF';
  const formData=new FormData(form);
  const file=formData.get('audioFile');
  if(!file || !file.size){notice.textContent='Please choose an audio file.';notice.style.color='#ff3c3c';return;}
  if(file.size>25*1024*1024){notice.textContent='File too large. Max upload size is 25MB.';notice.style.color='#ff3c3c';return;}
  try{
    if(!auth.currentUser) await signInAnonymously(auth);
    ...
    await addDoc(collection(db,'radio_submissions'),{artistName:formData.get('artistName')||'',email:formData.get('email')||'',trackTitle:formData.get('trackTitle')||'',artistLink:formData.get('artistLink')||'',producerCredits:formData.get('producerCredits')||'',genre:formData.get('genre')||'',copyrightDeclaration:formData.get('copyrightDeclaration')||'',rightsConfirm:!!formData.get('rightsConfirm'),audioUrl:downloadURL,fileType:file.type||'audio/mpeg',fileName:file.name||safeName,storagePath:fileRef.fullPath,status:'pending',featured:false,reviewNotes:'',approvedFor:[],submittedByUid:auth.currentUser?.uid||null,submittedByEmail:auth.currentUser?.email||formData.get('email'),isAnonymousSubmission:!!auth.currentUser?.isAnonymous,createdAt:serverTimestamp(),reviewedAt:null});
    form.reset(); modal.classList.remove('open'); notice.textContent='Submitted. Your track is pending admin review.'; notice.style.color='#00cc66';
  }catch(error){console.error(error);notice.textContent='Submission failed: '+(error.message||'Verify Firebase config and rules.');notice.style.color='#ff3c3c';}
});
...
async function resolveTrackUrl(track){
  ...
  if(track.storagePath){
    ...
    track.audioUrl=url;
    if(track.id && track.collectionName) updateDoc(doc(db,track.collectionName,track.id),{audioUrl:url}).catch(console.warn);
    return url;
  }
...
function renderApprovedTracks(){
  ...
  if(!filtered.length){approvedList.innerHTML='<div class="channel">No approved tracks in this category yet.</div>';return;}
