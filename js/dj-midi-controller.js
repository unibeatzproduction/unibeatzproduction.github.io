...
(function(){
  ...
  var DJMIDI = {
    ...
    defaultActions:{
      ...
      PAD_6:'NEXT_BEAT',
      PAD_7:'REPLAY_BEAT',
      FADER_71:'BEAT_VOLUME',
      FADER_72:'DJ_MIC_VOLUME',
      FADER_73:'ARTIST_MIC_VOLUME',
      MASTER_VOLUME_FADER_79:'MASTER_VOLUME',
      POT_20:'FILTER',
      POT_21:'ECHO',
      POT_22:'REVERB',
      POT_23:'CROWD_FX_LEVEL'
    }
  };
  ...
  function loadCustomMap(){
    try{
      ...
      DJMIDI.customMap = raw ? JSON.parse(raw) : {};
    }catch(e){ DJMIDI.customMap = {}; }
  }

  function saveCustomMap(){
    try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(DJMIDI.customMap || {})); }catch(e){}
  }

  function midiKey(status,data1){
    return String(status) + ':' + String(data1);
  }
  ...
  function resolvedAction(msg){
    var key = midiKey(msg.status,msg.data1);
    if(DJMIDI.customMap && DJMIDI.customMap[key]) return DJMIDI.customMap[key].action;
    var fallback = DJMIDI.defaultActions[msg.action];
    return fallback || msg.action;
  }
  ...
  function handleMidiMessage(e){
    ...
    if(DJMIDI.learning){
      DJMIDI.customMap[msg.key] = {
        action:DJMIDI.learning,
        status:status,
        data1:data1,
        type:msg.type,
        device:msg.device,
      ...
      saveCustomMap();
      toast('🎛️ MIDI learned: '+DJMIDI.learning);
      log('LEARNED', DJMIDI.learning, '=>', msg.key, msg);
      DJMIDI.learning = null;
      renderMonitor(msg);
      renderLearnPanel();
      return;
    }
  ...
  function consoleAction(msg){
    // Console-log only. Real DJ control wiring comes after testing.
    if(msg.isMasterVolume || msg.resolvedAction === 'MASTER_VOLUME') log('MASTER VOLUME:', msg.value);
    else log('ACTION:', msg.resolvedAction, 'VALUE:', msg.value);
  }
  ...
  function renderMonitor(msg){
    ...
    panel.innerHTML = '<div style="font-size:.48rem;letter-spacing:2px;color:#40D0FF;margin-bottom:5px;">DJ MIDI MONITOR</div>'+
      ...
      '<b style="color:#F0C040;">'+msg.resolvedAction+'</b><br>'+
      'Raw: '+msg.action+'<br>'+
      ...
      'Data1: '+msg.data1+'<br>'+
      'Value: '+msg.data2+'<br>'+
      (DJMIDI.learning ? '<span style="color:#F0C040;">Learning: '+DJMIDI.learning+'</span><br>' : '')+
  ...
  function renderLearnPanel(){
    ...
    var mapped = Object.keys(DJMIDI.customMap || {}).map(function(k){
      var m = DJMIDI.customMap[k];
      return '<div style="border:1px solid rgba(64,208,255,.24);border-radius:10px;padding:8px;margin-top:6px;background:rgba(255,255,255,.035);"><b style="color:#F0C040;">'+m.action+'</b><br><span style="font-size:.68rem;color:rgba(240,237,232,.72);">'+k+' · '+(m.type||'control')+'</span></div>';
    }).join('');

    var buttons = DEFAULT_ACTIONS.map(function(a){
      return '<button style="padding:7px 8px;border-radius:9px;border:1px solid rgba(64,208,255,.35);background:rgba(64,208,255,.08);color:#fff;font-size:.62rem;letter-spacing:1px;" onclick="ubDjMidiController.learn(\''+a+'\')">'+a.replace(/_/g,' ')+'</button>';
    }).join('');

    panel.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px;"><div><div style="font-size:.48rem;letter-spacing:2px;color:#40D0FF;">MIDI LEARN</div><div style="font-family:Bebas Neue,Arial,sans-serif;font-size:1.55rem;letter-spacing:2px;color:#F0C040;line-height:1;">CUSTOM MAP</div></div><button style="border:0;background:transparent;color:#F0C040;font-size:1.2rem;" onclick="ubDjMidiController.hideLearn()">×</button></div>'+
      '<div style="font-size:.75rem;color:rgba(240,237,232,.72);line-height:1.35;margin-bottom:9px;">Click an action, then move/press the controller control you want assigned. Saved on this device.</div>'+
      '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:7px;">'+buttons+'</div>'+
      '<button style="width:100%;margin-top:10px;padding:9px;border-radius:10px;border:1px solid rgba(201,168,76,.5);background:rgba(201,168,76,.12);color:#F0C040;font-weight:900;" onclick="ubDjMidiController.clearMap()">CLEAR CUSTOM MAP</button>'+
      '<div style="margin-top:10px;font-size:.48rem;letter-spacing:2px;color:#40D0FF;">SAVED MAPPINGS</div>'+
      (mapped || '<div style="margin-top:7px;color:rgba(240,237,232,.6);font-size:.75rem;">No custom mappings yet. FLkey default map still works.</div>')+
      (DJMIDI.learning ? '<div style="margin-top:9px;color:#F0C040;font-size:.8rem;">Waiting for control: '+DJMIDI.learning+'</div>' : '');
  }

  function showLearn(){
    renderLearnPanel();
    ensureLearnPanel().style.display = 'block';
  }
  ...
  function learn(action){
    DJMIDI.learning = action;
    showLearn();
    toast('Move/press control for '+action.replace(/_/g,' '));
    renderLearnPanel();
  }
  ...
  async function start(){
    ...
    try{
      ...
      DJMIDI.access.onstatechange = function(ev){
        ...
        if(ev.port.type === 'input' && ev.port.state === 'connected') ev.port.onmidimessage = handleMidiMessage;
      };
      ensureMonitor();
  ...
  setInterval(function(){
    var monitor = document.getElementById('ubDjMidiMonitor');
    if(monitor && !isDjArea()) monitor.style.display = 'none';
    var learnPanel = document.getElementById('ubDjMidiLearnPanel');
    if(learnPanel && !isDjArea()) learnPanel.style.display = 'none';
  },1500);
})();