...
// LAZY LOAD — does NOT init on startup. Only activates when DJ role is joined.
// This prevents blocking the PWA beforeinstallprompt event on page load.
...
(function(){
  ...
  var DJMIDI = {
    enabled:false, access:null, last:null, learning:null, customMap:{},
    ignoredClockCount:0,
  ...
  function handleMidiMessage(e){
    ...
    if(isSilentMidi(status,data1,data2)){ DJMIDI.ignoredClockCount++; return; }
    var msg=classify(status,data1,data2);
    msg.action=actionName(msg); msg.resolvedAction=resolvedAction(msg);
    ...
    if(DJMIDI.learning){
      ...
      saveCustomMap(); toast('🎛️ MIDI learned: '+DJMIDI.learning);
  ...
  function renderLearnPanel(){
    ...
    panel.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px;"><div><div style="font-size:.48rem;letter-spacing:2px;color:#40D0FF;">MIDI LEARN</div><div style="font-family:Bebas Neue,Arial,sans-serif;font-size:1.55rem;letter-spacing:2px;color:#F0C040;line-height:1;">CUSTOM MAP</div></div><button style="border:0;background:transparent;color:#F0C040;font-size:1.2rem;" onclick="ubDjMidiController.hideLearn()">×</button></div><div style="font-size:.75rem;color:rgba(240,237,232,.72);line-height:1.35;margin-bottom:9px;">Click an action, then move/press the control to assign it.</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:7px;">'+buttons+'</div><button style="width:100%;margin-top:10px;padding:9px;border-radius:10px;border:1px solid rgba(201,168,76,.5);background:rgba(201,168,76,.12);color:#F0C040;font-weight:900;" onclick="ubDjMidiController.clearMap()">CLEAR CUSTOM MAP</button><div style="margin-top:10px;font-size:.48rem;letter-spacing:2px;color:#40D0FF;">SAVED MAPPINGS</div>'+(mapped||'<div style="margin-top:7px;color:rgba(240,237,232,.6);font-size:.75rem;">No custom mappings yet.</div>')+(DJMIDI.learning?'<div style="margin-top:9px;color:#F0C040;font-size:.8rem;">Waiting for control: '+DJMIDI.learning+'</div>':'');
  }
  ...
  // ── LAZY INIT — only fires when DJ role joined ──
  var _started = false;
  async function start(){
    ...
    _started = true;
    loadCustomMap();
    if(!navigator.requestMIDIAccess){ log('Web MIDI not supported. Chrome/Edge desktop recommended.'); return; }
    try{
  ...
  // ── Hook into DJ join functions after page loads ──
  function hookDjJoin(){
    // Hook connectBattleLive / joinLiveBattleAs
    var fns = ['connectBattleLive','joinLiveBattleAs'];
    fns.forEach(function(name){
      var orig = window[name];
      if(typeof orig === 'function' && !orig._midiHooked){
        window[name] = function(role){
          if(String(role||'').toLowerCase().indexOf('dj') > -1) start();
          return orig.apply(this, arguments);
        };
        window[name]._midiHooked = true;
      }
    });
    // Hook joinCypher
    var origCypher = window.joinCypher;
    if(typeof origCypher === 'function' && !origCypher._midiHooked){
      window.joinCypher = function(role){
        if(String(role||'').toLowerCase() === 'dj') start();
        return origCypher.apply(this, arguments);
      };
      window.joinCypher._midiHooked = true;
    }
  }

  // Wait for page scripts then hook
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(hookDjJoin, 1000); });
  } else {
    setTimeout(hookDjJoin, 1000);
  }
  setTimeout(hookDjJoin, 2500);

  // Hide monitor when not in DJ area
  ...
  // ── DO NOT call start() here — lazy only ──
